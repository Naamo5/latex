...
From the \ac{FPD} equations (\ref{appendix:eq:rigid}) it is possible to isolate the inertial parameters of the item grasped by the tool using the following matrix equation (see $\left[ \bullet . \right]$ and $\left[. \times \right]$ notation in Section \ref{notation:matrix}).
...
From the equation of the FDP (\ref{appendix:notation:fpd}) on \{2\} at point $A$:
...
From the equation of the FDP (\ref{appendix:notation:fpd}) on \{1\} at point A:
...
The solution parameters of $\theta$ can be computed with the measured force $f$. From the FDP on \{1\} (\ref{appendix:rmsd:fpd1}), the force equation within the assumptions becomes:
